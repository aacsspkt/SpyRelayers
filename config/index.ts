	...
	privateKeys: [
		{
			...
			privateKeys: [
				[
					90, 136, 26, 232, 164, 37, 69, 48, 61, 93, 186, 200, 128, 201, 96, 142, 27, 94, 87, 124, 177, 166, 93, 60,
					193, 231, 146, 80, 57, 104, 136, 7, 53, 8, 189, 218, 126, 111, 209, 146, 40, 66, 89, 174, 123, 64, 84, 69,
					164, 16, 180, 245, 117, 243, 94, 138, 214, 87, 155, 227, 96, 233, 202, 12,
				],
			],
		},
		{
...
const tokenBridgeEnvironment =
	"SPY_SERVICE_FILTERS=" +
	JSON.stringify(tokenBridge.emmiterAddress) +
	"\n" +
	"SUPPORTED_CHAINS=" +
	JSON.stringify(tokenBridge.supportedChains) +
	"\n" +
	"SUPPORTED_TOKENS=" +
	JSON.stringify(tokenBridge.supportedTokens) +
	"\n" +
	"PRIVATE_KEYS=" +
	JSON.stringify(tokenBridge.privateKeys) +
	"\n";

const zebecBridgeEnvironment =
	"SPY_SERVICE_FILTERS=" +
	JSON.stringify(zebecBridge.emmiterAddress) +
	"\n" +
	"PRIVATE_KEYS=" +
	JSON.stringify(zebecBridge.privateKeys) +
	"\n";

const file1 = "tokenBridge.env";
const file2 = "zebecBridge.env";

fs.writeFileSync(file1, tokenBridgeEnvironment, "utf-8");
fs.writeFileSync(file2, zebecBridgeEnvironment, "utf-8");
