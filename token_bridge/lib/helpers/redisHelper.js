...
function createSourceToTargetMap(knownChainIds) {
    const sourceToTargetMap = {};
    for (const sourceKey of knownChainIds) {
        sourceToTargetMap[sourceKey] = {};
...
exports.createSourceToTargetMap = createSourceToTargetMap;
async function incrementSourceToTargetMap(key, redisClient, parse_vaa, sourceToTargetMap) {
    const parsedKey = storeKeyFromJson(key);
    const si_value = await redisClient.get(key);
    if (!si_value) {
        return;
    }
    const parsedPayload = (0, wormhole_sdk_1.parseTransferPayload)(Buffer.from(parse_vaa((0, wormhole_sdk_1.hexToUint8Array)(storePayloadFromJson(si_value).vaa_bytes)).payload));
    if (sourceToTargetMap[parsedKey.chain_id]?.[parsedPayload.targetChain] !== undefined) {
        sourceToTargetMap[parsedKey.chain_id][parsedPayload.targetChain]++;
    }
}
exports.incrementSourceToTargetMap = incrementSourceToTargetMap;
async function monitorRedis(metrics) {
    ...
    while (true) {
        ...
        if (!redisClient) {
            ...
            try {
                await redisClient.select(RedisTables.INCOMING);
                const incomingSourceToTargetMap = createSourceToTargetMap(knownChainIds);
                for await (const si_key of redisClient.scanIterator()) {
                    // @ts-ignore
                    incrementSourceToTargetMap(si_key, redisClient, parse_vaa, incomingSourceToTargetMap);
                }
                for (const sourceKey of knownChainIds) {
                    for (const targetKey of knownChainIds) {
                        metrics.setRedisQueue(RedisTables.INCOMING, sourceKey, targetKey, incomingSourceToTargetMap[sourceKey][targetKey]);
                    }
                }
                await redisClient.select(RedisTables.WORKING);
                ...
                for await (const si_key of redisClient.scanIterator()) {
                    // @ts-ignore
                    incrementSourceToTargetMap(si_key, redisClient, parse_vaa, workingSourceToTargetMap);
                }
                for (const sourceKey of knownChainIds) {
                    for (const targetKey of knownChainIds) {
                        metrics.setRedisQueue(RedisTables.WORKING, sourceKey, targetKey, workingSourceToTargetMap[sourceKey][targetKey]);
                    }
                }
            }
            catch (e) {
