...
(0, globals_1.test)("should pull Terra Classic token balances", async () => {
    if (!(terraClassicChainConfig.terraChainId &&
        terraClassicChainConfig.terraCoin &&
        terraClassicChainConfig.terraGasPriceUrl &&
        terraClassicChainConfig.terraName)) {
        throw new Error("Terra Classic relay was called without proper instantiation.");
    }
    const lcdConfig = {
    ...
    const localAddresses = await (0, walletMonitor_1.calcLocalAddressesTerra)(lcd, supportedTokens, terraClassicChainConfig);
    expect(localAddresses.length).toBeGreaterThan(0);
    for (const tokenAddress of localAddresses) {
        const balance = await (0, walletMonitor_1.pullTerraBalance)(lcd, terraClassicChainConfig.tokenBridgeAddress, tokenAddress, wormhole_sdk_1.CHAIN_ID_TERRA);
        console.log(balance);
        expect(balance).toBeDefined();
    }
});
//# sourceMappingURL=walletMonitor.test.js.map