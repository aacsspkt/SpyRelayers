...
export class TokenBridgeListener implements Listener {
	...
	verifyIsApprovedToken(payload: ParsedTransferPayload): boolean {
		...
		try {
			originAddressNative = tryHexToNativeString(payload.originAddress, payload.originChain);
		} catch (e: any) {
			return false;
		}
	...
	public async parsePayload(rawPayload: Uint8Array): Promise<ParsedTransferPayload> {
	...
	public async validate(rawVaa: Uint8Array): Promise<ParsedVaa<ParsedTransferPayload> | string> {
	...
	public async process(rawVaa: Uint8Array): Promise<void> {
		...
		const validationResults: ParsedVaa<ParsedTransferPayload> | string = await this.validate(rawVaa);
