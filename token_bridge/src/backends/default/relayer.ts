...
export class TokenBridgeRelayer implements Relayer {
	...
	async process(key: string, privateKey: any, relayLogger: ScopedLogger, metrics: PromHelper): Promise<void> {
		...
		try {
			...
			try {
				...
				const transferPayload = parseTransferPayload(Buffer.from(parsedVAA.payload));
	...
	async runAuditor(workerInfo: WorkerInfo, metrics: PromHelper): Promise<void> {
		const auditLogger = getScopedLogger([`audit-worker-${workerInfo.targetChainName}-${workerInfo.index}`]);
		while (true) {
			try {
	...
	async relay(
		...
		if (parsedVAA.payload[0] === 1) {
			...
			if (isEVMChain(transferPayload.targetChain)) {
				...
				try {
					nativeOrigin = tryHexToNativeString(transferPayload.originAddress, transferPayload.originChain);
				} catch (e: any) {
					return {
						status: Status.Error,
			...
			if (transferPayload.targetChain === CHAIN_ID_SOLANA) {
				...
				const retVal = await relaySolana(chainConfigInfo, signedVAA, checkOnly, walletPrivateKey, logger, metrics);
				if (retVal.redeemed) {
			...
			if (isTerraChain(transferPayload.targetChain)) {
				let rResult: RelayResult = { status: Status.Error, result: "" };
				const retVal = await relayTerra(chainConfigInfo, signedVAA, checkOnly, walletPrivateKey, logger, metrics);
				if (retVal.redeemed) {
					rResult.status = Status.Completed;
				}
				rResult.result = retVal.result;
