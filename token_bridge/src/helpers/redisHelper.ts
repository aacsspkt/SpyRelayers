...
export function storeKeyFromParsedVAA(parsedVAA: ParsedVaa<ParsedTransferPayload>): StoreKey {
	return {
...
export function createSourceToTargetMap(knownChainIds: ChainId[]): SourceToTargetMap {
...
export async function incrementSourceToTargetMap(
	...
	const parsedPayload = parseTransferPayload(
		Buffer.from(parse_vaa(hexToUint8Array(storePayloadFromJson(si_value).vaa_bytes)).payload),
	);
	if (sourceToTargetMap[parsedKey.chain_id as ChainId]?.[parsedPayload.targetChain] !== undefined) {
		sourceToTargetMap[parsedKey.chain_id as ChainId][parsedPayload.targetChain]++;
	}
}
