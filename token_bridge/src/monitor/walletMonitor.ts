...
export async function pullTerraBalance(
	...
	try {
		...
		return {
			...
			balanceFormatted: formatUnits(balanceInfo?.balance?.toString() || "0", tokenInfo.decimals),
...
async function pullSolanaTokenBalances(chainInfo: ChainConfigInfo, privateKey: Uint8Array): Promise<WalletBalance[]> {
...
async function pullEVMNativeBalance(chainInfo: ChainConfigInfo, privateKey: string): Promise<WalletBalance[]> {
...
async function pullSolanaNativeBalance(chainInfo: ChainConfigInfo, privateKey: Uint8Array): Promise<WalletBalance[]> {
	const keyPair = Keypair.fromSecretKey(privateKey);
	const connection = new Connection(chainInfo.nodeUrl);
	const fetchAccounts = await getMultipleAccountsRPC(connection, [keyPair.publicKey]);
	...
	const amountSol = formatUnits(fetchAccounts[0].lamports, WSOL_DECIMALS).toString();
...
async function calcLocalAddressesEVM(
	...
	const tokenBridge = Bridge__factory.connect(chainConfigInfo.tokenBridgeAddress, provider);
	...
	for (const supportedToken of supportedTokens) {
		...
		tokenAddressPromises.push(tokenBridge.wrappedAsset(supportedToken.chainId, hexToUint8Array(hexAddress)));
	}
...
async function pullAllEVMTokens(supportedTokens: SupportedToken[], chainConfig: ChainConfigInfo, metrics: PromHelper) {
	try {
		let provider = newProvider(chainConfig.nodeUrl, true) as ethers.providers.JsonRpcBatchProvider;
		...
		for (const privateKey of chainConfig.walletPrivateKey) {
			try {
				...
				const tokens = await Promise.all(
					localAddresses.map((tokenAddress) => getEthereumToken(tokenAddress, provider)),
				);
				const tokenInfos = await Promise.all(
					tokens.map((token) => Promise.all([token.decimals(), token.balanceOf(publicAddress), token.symbol()])),
				);
...
async function pullAllTerraBalances(
	...
	for (const privateKey of chainConfig.walletPrivateKey) {
		...
		for (const address of localAddresses) {
			const balance = await pullTerraBalance(lcd, walletAddress, address, chainConfig.chainId as TerraChainId);
			if (balance) {
